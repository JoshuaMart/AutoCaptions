...
class FFmpegConfig {
    ...
    async loadPresets() {
        ...
        try {
            ...
            this.showNotification('error', 'Loading Failed', 'Could not load presets. Using defaults.');
    ...
    renderPresets() {
        ...
        container.innerHTML = this.availablePresets.map(preset => `
    ...
    async selectPreset(presetName) {
        ...
        try {
            ...
            if (result.success && result.preset) {
                ...
                this.showNotification('success', 'Preset Loaded', `${result.preset.displayName} configuration loaded`);
            } else {
                throw new Error('Failed to load preset details');
            }
        } catch (error) {
            console.error('❌ Failed to load preset details:', error);
            this.showNotification('error', 'Loading Failed', 'Could not load preset configuration');
    ...
    generateConfigurationInterface(preset) {
        ...
        for (const [groupName, options] of Object.entries(groups)) {
            ...
            sectionsHTML += this.generateSectionHTML(groupName, sectionId, options, preset.defaults);
        }
    ...
    groupCustomizableOptions(customizableOptions) {
        ...
        customizableOptions.forEach(option => {
            ...
            } else if (key.includes('position') || key.includes('margin') || key.includes('offset')) {
    ...
    schedulePreviewUpdate() {
        ...
        this.previewTimeout = setTimeout(() => {
            if (this.currentPreset && !document.getElementById('preview-btn')?.disabled) {
                this.generatePreview();
            }
        }, 1500);
    }
    ...
    async generatePreview() {
        ...
        if (!config) {
            this.showNotification('error', 'Configuration Error', 'Please select a preset first');
            return;
        }
        ...
        try {
            ...
            this.showNotification('error', 'Preview Failed', 'Could not generate preview: ' + error.message);
    ...
    async generateFinalVideo() {
        ...
        try {
            this.showNotification('info', 'Processing', 'Generating your captioned video. This may take a few minutes...');
            ...
            this.showNotification('error', 'Generation Failed', 'Could not generate video: ' + error.message);
        } finally {
            // Re-enable button
    ...
    async refreshPresets() {
        ...
        await this.loadPresets();
        this.showNotification('success', 'Refreshed', 'Presets reloaded successfully');
    }
    ...
    showPreviewState(state) {
        ...
        // Hide all states
        [placeholder, image, loading, error].forEach(el => el?.classList.add('hidden'));
