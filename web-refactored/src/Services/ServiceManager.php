...
class ServiceManager
{
    ...
    public function __construct()
    {
        ...
        $this->defaultTimeout = $this->configManager->get(
            "services.timeout",
            10
        );
    }
    ...
    private function getServiceConfig(string $serviceName): ?array
    {
        ...
        if (!$config) {
            error_log(
                "ServiceManager: Effective configuration for service '{$serviceName}' not found via ConfigManager."
            );
            return null;
        }
        return $config;
    ...
    public function getServiceEndpointUrl(
        string $serviceName,
        string $endpointKey,
        array $pathParams = []
    ): ?string {
        ...
        if (!isset($config["endpoints"][$endpointKey])) {
            error_log(
                "ServiceManager: Endpoint key '{$endpointKey}' not found for service '{$serviceName}'."
            );
            return null;
        }
    ...
    public function healthCheck(string $serviceName): array
    {
        ...
        if (!$config) {
            return [
                "status" => "error",
                "statusCode" => null,
                "message" => "Configuration for service '{$serviceName}' not found.",
                "details" => null,
            ];
        }
        ...
        if (!$healthEndpoint) {
            return [
                "status" => "error",
                "statusCode" => null,
                "message" => "Health endpoint not configured for service '{$serviceName}'.",
                "details" => null,
            ];
        }
        ...
        if (
            ...
            if (
                ...
                return [
                    "status" => "healthy",
                    "statusCode" => $response["statusCode"],
                    "message" => "Service '{$serviceName}' responded successfully (non-JSON).",
            ...
            return [
                "status" => "healthy",
                "statusCode" => $response["statusCode"],
                "message" => "Service '{$serviceName}' responded successfully.",
                "details" => $response["body"],
            ];
        }
    ...
    public function makeRequest(
        ...
        if (!$config) {
            return [
                ...
                "statusCode" => null,
                "body" => null,
                "error" => "Configuration for service '{$serviceName}' not found.",
                "headers" => null,
            ];
        }

        $url = $this->getServiceEndpointUrl(
            $serviceName,
            $endpointKey,
            $pathParams
        );
        if (!$url) {
            return [
                ...
                "statusCode" => null,
                "body" => null,
                "error" => "Could not build URL for service '{$serviceName}', endpoint '{$endpointKey}'.",
                "headers" => null,
            ];
        }
    ...
    private function executeRequest(
                ...
                if (!empty($files)) {
                    ...
                    foreach ($files as $key => $file) {
                        if (is_string($file) && file_exists($file)) {
                            ...
                            error_log(
                                "ServiceManager: Invalid file entry for key '{$key}' in executeRequest."
                            );
                        }
                    }
                    ...
                    foreach ($headers as $h) {
                        if (
                            stripos($h, "Content-Type: application/json") === 0
                        ) {
                            $isJsonRequest = true;
                            break;
                        }
        ...
        if ($responseContent === false) {
            return [
                ...
                "statusCode" => null,
                "body" => null,
                "error" => "cURL request failed: {$curlError} (Error #{$curlErrorNo})",
                "headers" => null,
            ];
        }
        ...
        if ($curlErrorNo) {
            return [
                ...
                "body" => null,
                "error" => "cURL Error ({$curlErrorNo}): {$curlError}",
        ...
        if (stripos($contentType, "application/json") !== false) {
            ...
            if (json_last_error() === JSON_ERROR_NONE) {
                ...
                error_log(
                    "ServiceManager: Failed to decode JSON response from {$url}. Error: " .
                        json_last_error_msg() .
                        ". Body: " .
